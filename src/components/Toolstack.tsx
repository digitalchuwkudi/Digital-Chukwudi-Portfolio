...
export function Toolstack() {
  return (
    <section className="py-20 px-6 sm:px-12 lg:px-24 relative overflow-hidden bg-black border-b border-white/5">
      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center text-center relative z-10 w-full">
        <div className="flex flex-col items-center mb-12 lg:mb-16">
          ...
          <h2 className="font-display text-[24px] sm:text-[28px] lg:text-4xl font-bold mb-4 uppercase tracking-tight">Tools & Platforms</h2>
        ...
        <div className="w-full grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 sm:gap-8 justify-items-center max-w-sm md:max-w-3xl lg:max-w-6xl mx-auto">
          {tools.map((tool) => (
             <div key={tool.name} className="group relative flex flex-col items-center gap-3 sm:gap-4">
                ...
                <span className="text-[10px] sm:text-[11px] lg:text-xs font-display uppercase tracking-wider text-soft-white/40 group-hover:text-soft-white transition-colors">{tool.name}</span>
             </div>
          ))}
        </div>
      </div>
